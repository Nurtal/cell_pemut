...
def get_pop_to_voisin_norm(pop_to_voisin, cell_to_pop):
    """ """

    # get pop to count
    pop_to_count = {}
    for cell in cell_to_pop:
        pop = cell_to_pop[cell]
        if pop not in pop_to_count:
            pop_to_count[pop] = 1
        else:
            pop_to_count[pop] += 1
    ...
    pop_to_voisin_norm = {}
    for pop in pop_to_voisin:
        nb_cell_in_pop = pop_to_count[pop]
        pop_to_voisin_norm[pop] = {}
        
        for voisin in pop_to_voisin[pop]:
            voisin_count = pop_to_voisin[pop][voisin]
            voisin_count_norm = float(voisin_count) / float(nb_cell_in_pop)
            pop_to_voisin_norm[pop][voisin] = voisin_count_norm

    return pop_to_voisin_norm            
        ...
        for sf in glob.glob(f"{folder}/*.csv"):
            if row['file'] == sf.split("\\")[1]:
                ...
                if row['Groupe'] not in class_to_file:
                    ...
                    class_to_file[row['Groupe']] = [file_path]
                else:
...
def generate_proximity_matrix_heatmap(pop_to_voisin, fig_name):
    ...
    for k1 in pop_to_voisin:
        ...
        percentages = []
        for v in vector:
            if str(v) != 'nan' and total != 0:
                p = v / total *100
            elif str(v) != 'nan' and total == 0:
                p = 0
            else:
                p = np.nan
...
def display_multiclass_bar(group_to_voisin, population_title):
    """ """

    data = []
    group_list = []
    for group in group_to_voisin:
        data.append(group_to_voisin[group][population_title])
